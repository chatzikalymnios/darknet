...
void validate_coco(char *cfg, char *weights)
{
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            int h = val[t].h;
            int nboxes = 0;
            detection *dets = get_network_boxes(net, w, h, thresh, 0, 0, 0, 0, &nboxes);
...
void validate_coco_recall(char *cfgfile, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        int nboxes = 0;
        detection *dets = get_network_boxes(net, orig.w, orig.h, thresh, 0, 0, 1, 0, &nboxes);
...
void test_coco(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
        ...
        int nboxes = 0;
        detection *dets = get_network_boxes(net, 1, 1, thresh, 0, 0, 0, 0, &nboxes);
